...
extension ASCollectionView where SectionID == Int
{
     ...
     */
    public init(selectedItems: Binding<IndexSet>? = nil,
                shouldSelectItem: @escaping (Int, IndexPath.Element) -> Bool = { (_,_) in true },
                shouldDeselectItem: @escaping (Int, IndexPath.Element) -> Bool = { (_,_) in true },
                section: Section)
    {
        self.shouldSelect = shouldSelectItem
        self.shouldDeselect = shouldDeselectItem
        sections = [section]
    ...
    {
        self.init(data: data, dataID: \.id, selectedItems: selectedItems,
                  shouldSelectItem: shouldSelectItem, shouldDeselectItem: shouldDeselectItem,
                  contentBuilder: contentBuilder)
    }
}
...
public struct ASCollectionView<SectionID: Hashable>: UIViewControllerRepresentable, ContentSize
{
    ...
    var shouldDeselect: (Int, IndexPath.Element) -> Bool = { (_,_) in true}
     ...
     */
    @inlinable public init(selectedItems: Binding<[SectionID: IndexSet]>? = nil,
                           sections: [Section])
    {
    ...
    public class Coordinator: ASCollectionViewCoordinator
    {
        ...
        public func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
            let itemGroup = parent.sections[indexPath.section].id
            return parent.shouldSelect(itemGroup as! Int, indexPath.item)
        }

        public func collectionView(_ collectionView: UICollectionView, shouldDeselectItemAt indexPath: IndexPath) -> Bool {
            let itemGroup = parent.sections[indexPath.section].id
            return parent.shouldDeselect(itemGroup as! Int, indexPath.item)
        }
